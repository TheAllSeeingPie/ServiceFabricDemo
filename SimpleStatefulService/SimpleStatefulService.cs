            ...
            return new[]
{
                new ServiceReplicaListener(initParams =>
                    new ServiceRemotingListener<ISimpleStatefulService>(initParams, this))
            };
        }
        ...
        private const string QueueName = "SimpleStatefulServiceQueue";
        ...
        async Task ISimpleStatefulService.SetString(string value)
        {
            var queue = await StateManager.GetOrAddAsync<IReliableQueue<string>>(QueueName);
            using (var tx = StateManager.CreateTransaction())
            {
